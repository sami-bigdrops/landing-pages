...
// lp-core useUtmParams stores these aliases in cookies
...
export function collectTrackingParams(request: NextRequest): Map<string, string> {
  ...
  // 1. Read from URL query string first
  for (const key of TRACKED_UTM_KEYS) {
    const value = request.nextUrl.searchParams.get(key)?.trim()
    if (value) collected.set(key, value)
  }

  // 2. Fall back to cookies for any keys not found in URL
  //    Check both the raw key name and the lp-core alias (subid1/2/3)
  for (const key of TRACKED_UTM_KEYS) {
