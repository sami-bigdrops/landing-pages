...
export async function POST(request: NextRequest) {
  try {
    ...
    const {
      PRODUCT, PROP_ST, PROP_ZIP, PROP_DESC,
      SPEC_HOME, BUY_TIMEFRAME,
      EST_VAL, DOWN_PMT,
      LOAN_TYPE, CRED_GRADE, VA_STATUS,
    ...
    console.log("[submit-buy-home] payload:", JSON.stringify({
      PRODUCT, PROP_ST, PROP_ZIP, PROP_DESC, SPEC_HOME, BUY_TIMEFRAME,
      EST_VAL, DOWN_PMT, LOAN_TYPE, CRED_GRADE, VA_STATUS,
      FHA_BANK_FORECLOSURE, ANNUAL_VERIFIABLE_INCOME, NUM_MORTGAGE_LATES,
      EMAIL, FNAME, LNAME, ADDRESS, CITY, STATE, ZIP, PRI_PHON,
      subid1, subid2, subid3, ip,
    }, null, 2))

    const hasLeadProsper =
      process.env.HP_LEADPROSPER_CAMPAIGN_ID &&
      process.env.HP_LEADPROSPER_SUPPLIER_ID &&
      process.env.HP_LEADPROSPER_API_KEY &&
      process.env.HP_LEADPROSPER_API_URL

    if (hasLeadProsper) {
      const lpPayload = {
        lp_campaign_id:  process.env.HP_LEADPROSPER_CAMPAIGN_ID,
        lp_supplier_id:  process.env.HP_LEADPROSPER_SUPPLIER_ID,
        lp_key:          process.env.HP_LEADPROSPER_API_KEY,
        lp_subid1:       subid1,
        lp_subid2:       subid2,
        lp_subid3:       subid3,

        PRODUCT:         String(PRODUCT ?? "PP_NEWHOME"),
        PROP_ST:         String(PROP_ST ?? "").trim(),
        PROP_ZIP:        String(PROP_ZIP ?? "").trim(),
        PROP_DESC:       String(PROP_DESC ?? "SINGLE_FAM").trim(),
        SPEC_HOME:       String(SPEC_HOME ?? "YES").trim(),
        BUY_TIMEFRAME:   String(BUY_TIMEFRAME ?? "60_DAYS").trim(),
        CRED_GRADE:      String(CRED_GRADE ?? "GOOD").trim(),
        EST_VAL:         Number(EST_VAL ?? 0),
        DOWN_PMT:        Number(DOWN_PMT ?? 0),
      ...
      console.log("[submit-buy-home] LeadProsper payload:", JSON.stringify(lpLog, null, 2))

      const apiResponse = await fetch(process.env.HP_LEADPROSPER_API_URL!, {
        method: "POST",
      ...
      console.log("[submit-buy-home] LeadProsper raw response:", rawText)
    ...
    console.error("[submit-buy-home] error:", err)
