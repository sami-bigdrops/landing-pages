  ...
  try {
    ...
    if (hasLeadProsper) {
      const formData = {
        ...
        home_size: String(homeSize ?? "").trim(),
        roof_shape: String(roofShape ?? "").trim(),
        planning_process: String(planningProcess ?? "").trim(),
        has_attic: String(hasAttic ?? "").trim(),
        has_roof_leaks: String(hasRoofLeaks ?? "").trim(),
        has_metal_roof: String(hasMetalRoof ?? "").trim(),
        qualifies_for_discount: String(qualifiesForDiscount ?? "").trim(),
