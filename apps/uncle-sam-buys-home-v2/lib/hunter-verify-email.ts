...
export async function verifyEmailWithHunter(email: string): Promise<HunterVerifyResult> {
  ...
  for (let poll = 0; poll < 28; poll++) {
    ...
    if (res.status === 222) {
      ...
      if (smtpRetries > 4) {
        return {
          ok: false,
          message: "We could not complete email verification right now. Please try again in a moment.",
        }
      }
      await sleep(2000)
      continue
    ...
    if (res.status === 401 || res.status === 403 || res.status === 429) {
      return {
        ok: false,
        message: "Email verification is temporarily unavailable. Please try again in a few minutes.",
      }
    }
    ...
    if (!res.ok) {
      return { ok: false, message: "Email verification failed. Please try again." }
    }
    ...
    if (status === "invalid") {
      return {
        ok: false,
        message: "This email address does not appear to be valid or deliverable. Please check it and try again.",
      }
    }
    ...
    if (status === "disposable") {
      return {
        ok: false,
        message: "Temporary or disposable email addresses are not accepted. Please use a permanent email.",
      }
    }
    ...
    if (status === "unknown") {
      return {
        ok: false,
        message: "We could not verify this email. Please check the address or try another email.",
      }
    }
  ...
  return { ok: false, message: "Email verification timed out. Please try again." }
}
