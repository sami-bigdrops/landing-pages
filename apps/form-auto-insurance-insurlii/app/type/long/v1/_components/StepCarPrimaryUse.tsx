...
interface StepCarPrimaryUseProps {
  make: string
  model: string
  value: string
...
const OPTIONS = ["Commute", "Pleasure", "Business", "Farm"]

export function StepCarPrimaryUse({ make, model, value, onChange }: StepCarPrimaryUseProps) {
  return (
    <div>
      <h2
      ...
      >
        What is the primary use for your {make} {model}?
      </h2>

      <div className="grid grid-cols-1 gap-3 md:gap-4" role="radiogroup" aria-label="Vehicle primary use">
        {OPTIONS.map((option) => {
